...
import { collection, addDoc, Timestamp,updateDoc,doc } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-firestore.js";
...
const rankedIncreaseToast = document.getElementById('ranked-increase-alert');
const postingLimitModal = document.getElementById('posting_limit_modal')
const postIndex = 0;
...
rankedIncreaseToast.style.display = 'none';
...
async function getData() {  
  ...
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      updateDocumnet(getDataArr)
    });

    if (getDataArr.length > 0) {
      console.log('Match found');
      postCount++
      console.log(getDataArr[0]);
getDataArr.map(async(item)=>{
const newEmail = item.email;
const newFirstName = item.firstName;
const newUserId =  item.id;
const newLastName = item.lastName;
const newUid = item.uid;
const newUserAddress = item.userAddress;
const newUserPhoto = item.userPhoto;
const newTotalPosts  = postCount;
...
postLimitTotal = newTotalPosts;
localStorage.setItem('post-count',postCount);
console.log(postLimitTotal)
})
if(postLimitTotal === 3){
  // postLimit = false;
  toast.style.display = 'block';
  toast.innerHTML = 
  `<div class="fixed bottom-4 right-4 flex items-center space-x-3 bg-gradient-to-r from-green-400 via-green-500 to-green-600 text-white p-4 rounded-lg shadow-lg animate-fade-in-out mb-[100px]">
  <!-- SVG Icon -->
  <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-3-3v6m-3.293-2.293a1 1 0 011.414 1.414L12 12.414l1.293-1.293a1 1 0 111.414 1.414L13.414 14l1.293 1.293a1 1 0 01-1.414 1.414L12 15.414l-1.293 1.293a1 1 0 01-1.414-1.414L10.586 14 9.293 12.707a1 1 0 010-1.414z"></path>
  </svg>
  <!-- Animated Text -->
  <span class="text-sm xs:text-md sm:text-lg md:text-xl lg:text-2xl">
  Your profile rank is increased!
  </span>
  </div>`
  
     setTimeout(()=>{
       toast.style.display = 'none';
       // postingLimitModal.showModal();
      },3000)
...
async function updateDocumnet(arr){
  console.log(arr[0].email)
  const totalPosts = localStorage.getItem('post-count');
  console.log("🚀 ~ updateDocumnet ~ totalPosts:", totalPosts);
}
